...
module.exports.run = async (bot, message, args) => {
  ...
  if (!rMember.roles.has(gRole.id)) return message.reply("Sorry but that user doesn't have that role.");
  await (rMember.removeRole(gRole.id));

  message.delete(0);
  message.channel.send(rMember + " has lost the role " + gRole.name);
}
